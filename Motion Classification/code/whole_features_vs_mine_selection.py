...
def main():
    ...
    X1, y1 = mixAndShuffle(walk_data, other_data)

    walk_data1, other_data1 = genData(joints_all)

    print('walk_data len:', len(walk_data1))
    print('other_data len:', len(other_data1))
    X2, y2, = mixAndShuffle(walk_data1, other_data1)
    ...
    for i in range(20):
        ...
        print(num,' ', acc1, ' ', acc2)
        res.append([num, acc1, acc2])
    
    saveFile('diff_features_compare', res)
    ...
    # return score
    # return score1, score2, len(walk_data)
